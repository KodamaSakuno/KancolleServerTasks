...
var gameUrl = await connection.ExecuteScalarAsync<string>("SELECT value #>> '{}' FROM store WHERE name = 'game_url';");
...
var ra = await connection.ExecuteAsync("INSERT INTO api_start2_history VALUES(now(), @json::jsonb, convert_to(@json, 'UTF8')) ON CONFLICT DO NOTHING;", argument);
if (ra == 0)
{
...
await connection.ExecuteAsync(@"INSERT INTO current_api_start2 SELECT key, value FROM jsonb_each(@json::jsonb)
