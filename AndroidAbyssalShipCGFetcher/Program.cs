...
const string CallbackExchangeName = "AndroidAbyssalShipCGCallback";

rabbitMqChannel.ExchangeDeclare(CallbackExchangeName, ExchangeType.Topic, true);

const string DefaultCallbackQueueName = "AndroidAbyssalShipCGCallback";
...
callbackEventConsumer.Received += async (sender, e) =>
{
    ...
    await pg.ExecuteAsync("INSERT INTO android_abyssal_ship_cg VALUES(@ship, @version, @hash, @timestamp);", new
    {
        ship = shipId,
        version,
        timestamp,
